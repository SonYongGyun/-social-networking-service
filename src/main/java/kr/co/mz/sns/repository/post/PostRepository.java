...
public interface PostRepository extends JpaRepository<PostEntity, Long> {
    ...
    @Query("""
            SELECT new kr.co.mz.sns.dto.post.test.PostTestDto(p.seq, p.content, p.likes, p.createdAt, p.modifiedAt, p.createBy) 
            FROM PostEntity p""")
    @NotNull Page<PostTestDto> findAllWithPagingForTest(@NotNull Pageable pageable);
    ...
    @Query("SELECT p FROM PostEntity p "
            ...
            + "WHERE pf.seq = ("
            + "    SELECT MIN(pf2.seq) FROM PostFileEntity pf2 WHERE pf2.postSeq = p.seq)")
