...
//  public List<>
    ...
    public ResponseRelationshipDto blockFriend(Long friendSeq) {
        var relationship = friendRelationshipRepository
                .findByUserEntity_SeqAndFriendEntity_Seq(currentUserInfo.getSeq(), friendSeq)
                .orElseThrow(() -> new NotFoundException("No Relationships between this User."))
                .status(FR_BLOCKED);

        friendRelationshipRepository
                .findByUserEntity_SeqAndFriendEntity_Seq(friendSeq, currentUserInfo.getSeq())
                .ifPresentOrElse(
                        e -> e.status(FR_BLOCKED),
                        () -> {
                        });
